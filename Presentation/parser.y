...
void printST(){
    std::ofstream myfile;
    myfile.open("st.txt");
    std::cerr<<"\n\n\n"<<("name")<<":\t"<<("nodeType")<<"\t"<<("dataType")<<"\t"<<("Width")<<"\t\t"<<("value")<<"\n";
    myfile<<("name")<<":"<<("nodeType")<<""<<("dataType")<<""<<("value")<<"\n";
    for(auto t : mapper){
        string w;
        if(t.second.dataType == "int" ||t.second.dataType == "float")
            w="4";
        else if(t.second.dataType == "long" || t.second.dataType == "double")
            w="8";
        else if(t.second.dataType == "char")
            w="1";
        if(true){
            std::cerr<<(t.first)<<":\t"<<(t.second.nodeType)<<"\t"<<(t.second.dataType)<<"\t\t"<<(w)<<"\t\t"<<(t.second.value)<<"\n";
